# Beginner Tutorials

Start here after [First Program](../getting-started/first-program.md).

→ Printable: [Backer Checklist](../getting-started/backer-checklist.md)

---

## Tutorial index

| # | Tutorial | Time |
|---|----------|------|
| 1 | [Web Terminal](beginner/web-terminal.md) | 5 min |
| 2 | [Run Diagnostics](beginner/diagnostics.md) | 5 min |
| 3 | [Personalize Profile](beginner/personalize-profile.md) | 5 min |
| 4 | [Device Modes](beginner/device-modes.md) | 5 min |
| 5 | [Connect to Wi-Fi AP](../tutorials/wifi.md) | 10 min |
| 6 | [Read NFC tag](../tutorials/nfc.md) | 5 min |

---

## BOOT button

| Gesture | Action |
|---------|--------|
| Short press | Move ring menu pointer |
| Long press | Confirm |
| Double tap | Back / cancel |

→ [Buttons & Reset](../hardware/buttons.md)

---

## Next

- [CLI Reference](../software/cli-reference.md)
- [Quick Reference](../resources/quick-reference.md)
